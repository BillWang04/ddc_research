...
class OnsetNet:
    def __init__(self,
        ...
        #print (nfeats_conv)
        #print (feats_conv)
        feats_all = tf.concat([feats_conv, feats_other], 1)     # original: feats_all = tf.concat(1, [feats_conv, feats_other])
        ...
        if do_rnn:
            ...
            rnn_inputs = [tf.compat.v1.squeeze(input_, [1]) for input_ in rnn_inputs]
        ...
        logits = tf.compat.v1.squeeze(tf.nn.bias_add(tf.matmul(dnn_output, logit_w), logit_b), squeeze_dims=[1])
        prediction = tf.nn.sigmoid(logits)
        prediction_inspect = tf.reshape(prediction, [batch_size, rnn_nunroll])
        prediction_final = tf.compat.v1.squeeze(tf.slice(prediction_inspect, [0, rnn_nunroll - 1], [-1, 1]), squeeze_dims=[1])
