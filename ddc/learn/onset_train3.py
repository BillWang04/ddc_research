...
from onset_net2 import OnsetNet
from util import *
...
def main(_):
    ...
    model_config = {
        ...
        'cnn_init': tf.compat.v1.initializers.variance_scaling(scale=1.43, mode='fan_avg', distribution='uniform', dtype=dtype),#tf.uniform_unit_scaling_initializer(factor=1.43, dtype=dtype),  #factor=1.43
        ...
        'rnn_init': tf.compat.v1.initializers.random_uniform(minval=-5e-2, maxval=5e-2),#tf.random_uniform_initializer(-5e-2, 5e-2, dtype=dtype),
        'rnn_nunroll': FLAGS.rnn_nunroll,
        'rnn_keep_prob': FLAGS.rnn_keep_prob,
        'dnn_sizes': stride_csv_arg_list(FLAGS.dnn_sizes, 1, int),
        'dnn_init': tf.compat.v1.initializers.variance_scaling(scale=1.15, mode='fan_avg', distribution='uniform', dtype=dtype),#tf.uniform_unit_scaling_initializer(factor=1.15, dtype=dtype),   #factor=1.15
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
        ...
        if do_train:
            ...
            with tf.compat.v1.variable_scope('model_sp', reuse=None):
        ...
        if do_train_eval or do_eval or do_cnn_export:
            with tf.compat.v1.variable_scope('model_sp', reuse=do_train):
                ...
                model_early_stop_auprc = tf.compat.v1.train.Saver(tf.compat.v1.global_variables(), max_to_keep=None)
                model_early_stop_fscore = tf.compat.v1.train.Saver(tf.compat.v1.global_variables(), max_to_keep=None)
        ...
        if do_train:
            ...
            with open('Perplexities.txt', 'a') as p_file, open('Onset_Xentropies.txt', 'a') as x_file, open('Onset_AUPRCs.txt', 'a') as a_file, open('Onset_Fscores.txt', 'a') as f_file:
                ...
                while 1 == 0 or FLAGS.nepochs < 0 or batch_num < nbatches:
                    ...
                    batch_num += 1
                    # x_f = open('batch_xentropies.txt', 'a')
                    # x_f_str = str(batch_num) + ": " + str(batch_xentropy) + "\n"
                    # x_f.write(x_f_str)
                    ...
                    if batch_num % FLAGS.nbatches_per_ckpt == 0:
                        ...
                        model_saver.save(sess, ckpt_fp, global_step=global_step)
                    ...
                    if do_train_eval and batch_num % FLAGS.nbatches_per_eval == 0:
                        ...
                        if xentropy_avg_mean < eval_best_xentropy_avg:
                            ...
                            model_early_stop_xentropy_avg.save(sess, ckpt_fp, global_step=global_step)
                        ...
                        #     a_file.write('{},{},{}\n'.format(auprc_mean, eval_best_auprc, batch_num))
                        #     a_file.flush()
                        ...
                        if auprc_mean > eval_best_auprc:
                            ...
                            a_file.flush()
                            ckpt_fp = os.path.join(FLAGS.experiment_dir, 'onset_net_early_stop_auprc')
                            model_early_stop_auprc.save(sess, ckpt_fp, global_step=global_step)
                            eval_best_auprc = auprc_mean
                        ...
                        if fscore_mean > eval_best_fscore:
                            ...
                            f_file.write('{},{},{}\n'.format(fscore_mean, eval_best_fscore, batch_num))
                            f_file.flush()
                            ckpt_fp = os.path.join(FLAGS.experiment_dir, 'onset_net_early_stop_fscore')
                            model_early_stop_fscore.save(sess, ckpt_fp, global_step=global_step)
...
def eval_metrics_for_scores(y_true, y_scores, y_xentropies, y_scores_pkalgn, given_threshold_names=[], given_thresholds=[], return_prc=False):
    ...
    # calculate accuracy
    y_labels = np.zeros(y_scores_pkalgn.shape[0], dtype=np.int32)
    y_labels[predicted_steps] = 1
    accuracy = accuracy_score(y_true.astype(np.int32), y_labels)
