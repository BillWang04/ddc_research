import essentia
from essentia.standard import MonoLoader, FrameGenerator, Windowing, Spectrum, MelBands, BeatTrackerMultiFeature, OnsetDetection
import numpy as np
...
def extract_mel_feats(audio_fp, analyzers, fs=44100.0, nhop=512, nffts=[1024, 2048, 4096], log_scale=True):
    ...
    # print(type(audio_fp))
    # print(fs)
    loader = MonoLoader(filename=str(audio_fp), sampleRate=fs)
    samples = loader()
    # beats, confidence = BeatTrackerMultiFeature(samples)
    # print(confidence)
    # b = open("beats.txt", 'a')
    # b.write(str(beats)+'\n')
...
if __name__ == '__main__':
    ...
    for dataset_fp in args.dataset_fps:
        ...
        for json_fp in json_fps:
            ...
            # print("song feats: {}".format(song_feats))
