...
class OnsetChart(Chart):
    ...
    def sample_blanks(self, n, exclude_onset_neighbors=0, exclude_pre_onsets=True, exclude_post_onsets=True,
        ...
        if exclusion_params in self._blanks_memoized:
            ...
            blanks = self.blanks
            # print("blanks = " + str(len(blanks)))
            # print('exclude_onset_neighbors: {}'.format(exclude_onset_neighbors))
            if exclude_onset_neighbors > 0:
            ...
            if exclude_pre_onsets:
                ...
                # print("blanks after get_first = " + str(len(blanks)))
            if exclude_post_onsets:
                blanks -= set(range(self.get_last_onset(), self.nframes))
                # print("blanks after get_last = " + str(len(blanks)))
            if include_onsets:
                blanks |= self.onsets
                # print("blanks after include = " + str(len(blanks)))
        ...
        if n > len(blanks):
            ...
            with open('delete_these.txt', 'a') as f:
                f.write(str(self.song_metadata))
            return random.sample(blanks, len(blanks))
    ...
    def get_subsequence(self,
        ...
        return np.array(seq_feats_audio, dtype=dtype), np.array(seq_feats_other, dtype=dtype), np.array(seq_y, dtype=dtype)
...
class SymbolicChart(Chart):
    def __init__(self, song_metadata, song_features, frame_rate, metadata, annotations, pre=1, post=False):
        ...
        for i in range(len(annotations) - 1):
            ...
            self.seq_beat_phase.append(beat - np.floor(beat))
