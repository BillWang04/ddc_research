...
tf.compat.v1.flags.DEFINE_string('sym_rnn_pretrain_model_ckpt_fp', '', 'File path to model checkpoint with only sym weights')
tf.compat.v1.flags.DEFINE_string('model_ckpt_fp', '', 'File path to model checkpoint if resuming or eval')

# Features
tf.compat.v1.flags.DEFINE_string('sym_in_type', 'onehot', 'Either \'onehot\' or \'bagofarrows\'')
tf.compat.v1.flags.DEFINE_string('sym_out_type', 'onehot', 'Either \'onehot\' or \'bagofarrows\'')
tf.compat.v1.flags.DEFINE_integer('sym_narrows', 4, 'Number or arrows in data')
tf.compat.v1.flags.DEFINE_integer('sym_narrowclasses', 4, 'Number or arrow classes in data')
tf.compat.v1.flags.DEFINE_integer('sym_embedding_size', 32, '')
tf.compat.v1.flags.DEFINE_bool('audio_z_score', False, 'If true, train and test on z-score of validation data')
tf.compat.v1.flags.DEFINE_integer('audio_deviation_max', 0, '')
tf.compat.v1.flags.DEFINE_integer('audio_context_radius', -1, 'Past and future context per training example')
tf.compat.v1.flags.DEFINE_integer('audio_nbands', 0, 'Number of bands per frame')
tf.compat.v1.flags.DEFINE_integer('audio_nchannels', 0, 'Number of channels per frame')
tf.compat.v1.flags.DEFINE_bool('feat_meas_phase', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_meas_phase_cos', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_meas_phase_sin', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_beat_phase', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_beat_phase_cos', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_beat_phase_sin', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_beat_diff', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_beat_diff_next', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_beat_abs', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_time_diff', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_time_diff_next', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_time_abs', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_prog_diff', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_prog_abs', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_diff_feet', False, '')
tf.compat.v1.flags.DEFINE_bool('feat_diff_aps', False, '')
tf.compat.v1.flags.DEFINE_integer('feat_beat_phase_nquant', 0, '')
tf.compat.v1.flags.DEFINE_integer('feat_beat_phase_max_nwraps', 0, '')
tf.compat.v1.flags.DEFINE_integer('feat_meas_phase_nquant', 0, '')
tf.compat.v1.flags.DEFINE_integer('feat_meas_phase_max_nwraps', 0, '')
...
tf.compat.v1.flags.DEFINE_integer('nunroll', 1, '')
...
tf.compat.v1.flags.DEFINE_integer('cnn_dim_reduction_size', -1, '')
tf.compat.v1.flags.DEFINE_float('cnn_dim_reduction_keep_prob', 1.0, '')
tf.compat.v1.flags.DEFINE_string('cnn_dim_reduction_nonlin', '', '')
...
# Generate params
tf.compat.v1.flags.DEFINE_string('generate_fp', '', '')
tf.compat.v1.flags.DEFINE_string('generate_vocab_fp', '', '')
...
def main(_):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        if do_train:
            ...
            eval_best_perplexity = float('inf')
            p_file = open('Sym_perplexity.txt', 'a')
            x_file = open('Sym_xentropy.txt', 'a')
            a_file = open('Sym_accuracy.txt', 'a')
            while FLAGS.nepochs < 0 or batch_num < nbatches:
                ...
                if batch_num % batches_per_epoch == 0:
                    ...
                    p_file.write('Completed epoch {}'.format(epoch_num))
                ...
                if do_train_eval and batch_num % FLAGS.nbatches_per_eval == 0:
                    ...
                    perplexity_avg_mean = np.exp(xentropy_avg_mean)
                    if perplexity_avg_mean < eval_best_perplexity:
                        p_file.write('{},{},{}\n'.format(perplexity_avg_mean, eval_best_perplexity, batch_num))
                        p_file.flush()
