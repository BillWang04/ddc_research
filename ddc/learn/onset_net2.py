...
class OnsetNet:
    def __init__(self,
        ...
        # Input tensors
        ## TODO: Seems like batchsize sometimes not batchsize?
        ...
        if do_cnn:
            ...
            for i, ((ntime, nband, nfilt), (ptime, pband)) in enumerate(zip(cnn_filter_shapes, cnn_pool)):
                ...
                with tf.compat.v1.variable_scope(layer_name):
                    filters = tf.Variable(cnn_init([ntime, nband, nfilt_last, nfilt]), dtype=dtype, name='filters')

                    # filters = tf.get_variable('filters', [ntime, nband, nfilt_last, nfilt], initializer=cnn_init, dtype=dtype)
                    biases = tf.Variable(tf.constant_initializer(0.1)([nfilt]), dtype=dtype, name='biases')
        ...
        if do_dnn:
            ...
            for i, layer_size in enumerate(dnn_sizes):
                ...
                with tf.compat.v1.variable_scope(layer_name):
                    ...
                    dnn_w = tf.Variable(initial_value=dnn_init(shape=[last_layer_size, layer_size], dtype=dtype), name='W')
                    dnn_b = tf.Variable(initial_value=tf.constant_initializer(0.0)(shape=[layer_size], dtype=dtype), name='b')
        ...
        with tf.compat.v1.variable_scope('logit') as scope:
            logit_w = tf.Variable(
                initial_value=tf.random.normal([dnn_output_size, 1], stddev=1.0 / dnn_output_size, dtype=dtype),
                name='W'
            )
            logit_b = tf.Variable(
                initial_value=tf.constant(0.0, shape=[1], dtype=dtype),
                name='b'
            )
        ...
        prediction_final = tf.squeeze(tf.slice(prediction_inspect, [0, rnn_nunroll - 1], [-1, 1]), axis=[1])
        ...
        if mode == 'train':
            ...
            grads = tf.compat.v1.gradients(avg_neg_log_lhood, tvars)
    ...
    def assign_lr(self, sess, lr_new):
        lr_summary = tf.compat.v1.Summary(value=[
        tf.compat.v1.Summary.Value(
            tag='learning_rate',
            simple_value=float(lr_new)
            )
        ])
        
        # Perform the learning rate assignment
    ...
    def prepare_train_batch(self, charts, randomize_charts=False, **kwargs):
        ...
        if randomize_charts:
            ...
            print("Randomized batch shapes:")
            print(f"batch_feats_audio shape: {batch_feats_audio.shape}")
            print(f"batch_feats_other shape: {batch_feats_other.shape}")
            print(f"batch_targets shape: {batch_targets.shape}")
            print(f"batch_target_weights shape: {batch_target_weights.shape}")

            return batch_feats_audio, batch_feats_other, batch_targets, batch_target_weights
        else:
            chart = charts[random.randint(0, len(charts) - 1)]
            chart_nonsets = chart.get_nonsets()
            if exclude_kwargs.get('include_onsets', False):  # CHANGE THIS BACK TO False
                npos = 0
            ...
            print(f"chart_nonsets: {chart_nonsets}")
            print(f"npos: {npos}")
            print(f"nneg: {nneg}")
            ...
            # ... [rest of the code remains the same until final arrays] ...
            batch_feats_audio = []
            batch_feats_other = []
            batch_targets = []
            batch_target_weights = []
