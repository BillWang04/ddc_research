if __name__ == '__main__':
    ...
    chart_types = set([x for x in [x.strip() for x in args.chart_types.split(',')] if bool(x)])
    chart_difficulties = set([x for x in [x.strip() for x in args.chart_difficulties.split(',')] if bool(x)])
    substitutions = list([x for x in [x.strip() for x in args.substitutions.split(',')] if bool(x)])
    ...
    arrow_types = set([x for x in [x.strip() for x in args.arrow_types.split(',')] if bool(x)])
    arrow_types.add('0')
    ppms = set([int(x.strip()) for x in [x for x in args.ppms.split(',') if bool(x)]])
    permutations = set([x.strip() for x in [x for x in args.permutations.split(',') if bool(x)]])
